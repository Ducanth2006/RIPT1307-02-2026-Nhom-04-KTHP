...
import { Avatar, Dropdown, message, Popover } from 'antd';
    ...
    <div className="flex h-screen bg-[#f7f9fb] font-sans text-[#191c1e] overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-[#f7f9fb] border-r border-[#e4beba] h-full py-6 z-40 shrink-0">
        <div className="px-6 mb-8 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-[#d32f2f] text-white flex items-center justify-center font-bold text-lg">P</div>
          <div>
            ...
            return (
              <Link 
                ...
                className={`flex items-center gap-3 px-4 py-3 mx-2 my-0.5 rounded-lg transition-colors ${
                  isActive 
                  ? 'text-[#af101a] font-bold bg-[#ffdad6]/50' 
                  : 'text-[#5b403d] hover:bg-[#e0e3e5]'
                }`}
              >
            ...
            <Dropdown 
              menu={{ 
                items: [
                  { 
                    key: 'account', 
                    label: <Link to="/account" className="flex items-center gap-2"><User size={14} /> My Account</Link> 
                  },
                  { type: 'divider' },
                  { 
                    key: 'logout', 
                    label: <Link to="/login" className="flex items-center gap-2 text-red-600"><LogOut size={14} /> Logout</Link> 
                  }
                ] 
              }} 
              placement="bottomRight"
